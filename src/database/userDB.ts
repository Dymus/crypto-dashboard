...
export const getUserById = async (userId: string, options?) => {
  return UserModel.findById(userId, options).then((user) => {
    if (user) return user;
...
export const setUserAlertsInUserDB = async (userId: string, alerts) => {
  return UserModel.findByIdAndUpdate(userId, { alerts }, { new: true }).then((updatedUser) => {
...
export const removeAllAlertNotificationsFromUserDB = async (userId: string) => {
  return UserModel.findByIdAndUpdate(userId, { notifications: [] }, { new: true }).then((updatedUser) => {
