...
export const getGeminiTradesForAccount: RequestHandler = async (req, res, next) => {
  if (req.params.currencyCode.toLowerCase() === 'eth' || req.params.currencyCode.toLowerCase() === 'btc') {
    return Promise.all([
      ...
      async ([geminiEurResponse, geminiUsdResponse]) => {
        const orderDates = [];
        const exchangeRates = {};
        ...
        for (let i = 0; i < geminiUsdResponse.data.length; i++) {
          ...
          if (geminiUsdResponse.data[i].order_id in orders) {
            ...
            orders[geminiUsdResponse.data[i].order_id].orderFiatAmount +=
              ...
              exchangeRate; // MULTIPLY ORDER_FIAT_AMOUNT BY EUR EXCHANGE RATE
          } else {
        ...
        for (let i = 0; i < geminiEurResponse.data.length; i++) {
          if (geminiEurResponse.data[i].order_id in orders) {
            ...
            orders[geminiEurResponse.data[i].order_id].orderFiatAmount +=
              +geminiEurResponse.data[i].amount * +geminiEurResponse.data[i].price +
              +geminiEurResponse.data[i].fee_amount;
          } else {
