...
const updateHourlyNotifications = async () => {
  try {
    const usersWithAlerts = await UserModel.find({ alerts: { $nin: [null, new Object()] } });
    ...
    console.log('error while updating hourly notifications');
  }
};
...
const updateDailyNotifications = async () => {
  try {
    ...
    console.log('error while updating daily notifications');
  }
};

const updateWeeklyNotifications = async () => {
  try {
    const usersWithAlerts = await UserModel.find({ alerts: { $nin: [null, new Object()] } });
    if (usersWithAlerts.length > 0) {
    ...
    console.log('error while updating weekly notifications');
  }
};
...
const getHourlyCoingeckoData = async (users: DocumentType<User>[]) => {
  return (
    await axios.get(
      `https://api.coingecko.com/api/v3/coins/markets?vs_currency=eur&ids=${users
        .map((user) => {
...
const setUserHourlyNotifications = (user: DocumentType<User>, coingeckoData) => {
  ...
  for (const coingeckoName in user.alerts) {
    ...
    if (
      Math.abs(coinData.price_change_percentage_1h_in_currency) >= user.alerts[coingeckoName].hourPriceChange &&
      user.alerts[coingeckoName].hourPriceChange !== 0
    ) {
        ...
        message:
          coinData.price_change_percentage_1h_in_currency > 0
            ? `Hour price alert: ${coinData.name} went up ${
                Math.floor(coinData.price_change_percentage_1h_in_currency * 100) / 100
              }%`
            : `Hour price alert: ${coinData.name} went down ${
                Math.floor(coinData.price_change_percentage_1h_in_currency * 100) / 100
              }%`,
        wasViewed: false,
...
const setUserDailyNotifications = (user: DocumentType<User>, coingeckoData) => {
  ...
  for (const coingeckoName in user.alerts) {
    const coinData = coingeckoData.find((coin) => coin.id === coingeckoName);
    if (
      Math.abs(coinData.price_change_percentage_24h_in_currency) >= user.alerts[coingeckoName].dayPriceChange &&
      user.alerts[coingeckoName].dayPriceChange !== 0
    ) {
      newNotifications.push({
        id: uuid4(),
        token: coingeckoName,
        ...
        message:
          coinData.price_change_percentage_24h_in_currency > 0
            ? `Day price alert: ${coinData.name} went up ${
                Math.floor(coinData.price_change_percentage_24h_in_currency * 100) / 100
              }%`
            : `Day price alert: ${coinData.name} went down ${
                Math.floor(coinData.price_change_percentage_24h_in_currency * 100) / 100
              }%`,
        wasViewed: false,
...
const setUserWeeklyNotifications = (user: DocumentType<User>, coingeckoData) => {
  ...
  for (const coingeckoName in user.alerts) {
    const coinData = coingeckoData.find((coin) => coin.id === coingeckoName);
    if (
      Math.abs(coinData.price_change_percentage_7d_in_currency) >= user.alerts[coingeckoName].weekPriceChange &&
      user.alerts[coingeckoName].weekPriceChange !== 0
    ) {
      newNotifications.push({
        id: uuid4(),
        token: coingeckoName,
        ...
        message:
          coinData.price_change_percentage_7d_in_currency > 0
            ? `Week price alert: ${coinData.name} went up ${
                Math.floor(coinData.price_change_percentage_7d_in_currency * 100) / 100
              }%`
            : `Week price alert: ${coinData.name} went down ${
                Math.floor(coinData.price_change_percentage_7d_in_currency * 100) / 100
              }%`,
        wasViewed: false,
